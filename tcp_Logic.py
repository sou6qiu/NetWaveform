...
class TcpLogic(Tcp_ucpUi, Ui_MainWindow):
    def __init__(self):
    ...
    def socket_open_tcps(self):
        ...
        local_ip = self.ip_box.currentText()
    ...
    def accept_concurrency(self):
        ...
        while True:  # 无限等待连接
            try:
                ...
                statusbar_client_info = f'{addr[0]}:{addr[1]}'
                ...
                if recv_msg:
                    ...
                    if self.recv_hex_radio.isChecked():
                        ...
                        if show_client_info is True:
                            ...
                            connect_info = f'[Remote IP {addr[0]} Port: {addr[1]} ]\n'
    ...
    def socket_open_tcpc(self):
        ...
        """
        self.connect_button.setEnabled(False)
        remote_ip = self.ip_box.text()
        remote_port = self.port_edit.text()
